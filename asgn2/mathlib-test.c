...
void do_newton(int stat) {
    ...
    printf("sqrt_newton() = %16.15lf, M_SQRT2 = %16.15lf, diff = %16.15lf\n", sqr2_result,
        sqr2_math, sqr2_result - sqr2_math);
    if (stat) {
...
int main(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "abehmnrsv")) != -1) {
        switch (c) {
        case 'e': need_do_e = 1; break;
        case 'b': need_do_bbp = 1; break;
        case 'v': need_do_viete = 1; break;
        case 'r': need_do_euler = 1; break;
        case 'm': need_do_madhava = 1; break;
        case 'n': need_do_newton = 1; break;
        case 's': use_stats = 1; break;
        case 'a':
            need_do_e = 1;
            need_do_bbp = 1;
            need_do_viete = 1;
